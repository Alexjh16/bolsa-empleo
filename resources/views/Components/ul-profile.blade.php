<ul class="hidden items-center gap-4 sm:flex ">
    ...
    <li x-data="{ userDropDownIsOpen: false, openWithKeyboard: false }" x-on:keydown.esc.window="userDropDownIsOpen = false, openWithKeyboard = false"
        ...
        <button x-on:click="userDropDownIsOpen = ! userDropDownIsOpen" x-bind:aria-expanded="userDropDownIsOpen"
            x-on:keydown.space.prevent="openWithKeyboard = true" x-on:keydown.enter.prevent="openWithKeyboard = true"
            ...
            <img src="{{ route('photo.profile') }}?v={{ now()->timestamp }}" alt="User Profile"
        ...
        <ul x-cloak x-show="userDropDownIsOpen || openWithKeyboard" x-transition.opacity x-trap="openWithKeyboard"
            ...
            <li class="border-b border-neutral-300 ">
                <div class="flex flex-col px-4 py-2">
                    <span
                        class="text-sm font-medium text-neutral-900 ">{{ Auth::user()->first_name . ' ' . Auth::user()->last_name }}</span>
            ...
            <li><a href="/perfil"
                    class="block bg-neutral-50 px-4 py-2 text-sm text-neutral-600 hover:bg-neutral-900/5 hover:text-neutral-900 focus-visible:bg-neutral-900/10 focus-visible:text-neutral-900 focus-visible:outline-hidden ">Mi
                    Perfil</a>
            </li>
            <li>
                <a
